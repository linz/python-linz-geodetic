...
class AmbiguousSolution(RuntimeError):
    pass
...
class CovarianceMissing(RuntimeError):
    pass


class SinexFileError(RuntimeError):
    pass
...
class Reader(object):
    ...
    Solution = namedtuple(
        "Solution",
        "id code soln monument description llh startdate enddate crddate crddate_year xyz vxyz prmids covariance",
    )
        ...
        with open(self._filename, "w") as sf:
            ...
            for m in marks:
                ...
                if lon < 0:
                    lon += 360
                sf.write(
                    " {0:4.4s} {1:2.2s} {2:9.9s} C {3:22.22s} {4} {5} {6:7.1f}\n".format(
                        m.id,
                        m.code,
                        m.monument,
                        m.description,
            ...
            for m in marks:
                for s in m.solutions:
                    ...
                    for crd, value, prmno in zip(crds, s.xyz, s.params):
                        ...
                        sf.write(
                            " {0:5d} STA{1:1.1s}   {2:4.4s} {3:2.2s} {4:4.4s} {5} {6:4.4s} {7:1.1s} {8:21.14e} {9:11.5e}\n".format(
                                prmno,
                                crd,
                                m.id,
                                m.code,
                                s.solnid,
                                self._sinexEpoch(crddate),
                                "m",
                                "2",
                                value,
                                stderr,
